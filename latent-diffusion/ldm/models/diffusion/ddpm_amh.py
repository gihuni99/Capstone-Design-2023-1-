...
class LatentDiffusion(DDPM):
    ...
    def get_crowd(self, batch, key):
        crowd = batch[key]
        crowd = crowd.to(self.device)
        return crowd
    
    def shared_step(self, batch, **kwargs):
        x, c = self.get_input(batch, self.first_stage_key)
        crowd = self.get_crowd(batch, "crowd")
        # loss = self(x, c)
        loss = self(x, c, x_crowd=crowd)
        return loss
    ...
    @torch.no_grad()
    def to_arr(self, img, batch_size):
    ...
    @torch.no_grad()
    def counting(self, image, batch_size, fp):
    ...
    def p_losses(self, x_start, cond, t, noise=None, *args, **kwargs):
        x_crowd = list(kwargs.values())[0] # tensor, 배치 수 만큼 인티저
        # print("crowd:", x_crowd)
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise) # z -> z_T
        model_output = self.apply_model(x_noisy, t, cond) # z_T -> Z_T-1
        loss_dict = {}
        ...
        if self.parameterization == "x0":
            ...
            target = noise # z_T - z_T-1
            # x_recon = self.predict_start_from_noise(x_t=x_noisy, t=t, noise=model_output)
            # samples, _ = self.sample_log(cond=cond,batch_size=batch_size,ddim=True,
            #                                         ddim_steps=20, eta=1.)
                    ## 단순 시각화 용 ##
            # x_img = self.normalize_minus(self.to_gray(self.normalize_zero(x_start)))
            # x_arr = self.to_arr(x_img,batch_size)
            # crowd_x = self.counting(x_arr,batch_size, "x0_count.png")
            # ###################
            
            # ## crowd 예측 파트 ##
            # pred_img = self.normalize_minus(self.to_gray(self.normalize_zero(samples)))
            # pred_arr = self.to_arr(pred_img,batch_size)
            # crowd_pred = self.counting(pred_arr,batch_size, "pred_count.png")
            # #####################
            ...
            # ## Loss 계산 #########
            # error = (x_start - samples).abs().mean([1, 2, 3]).mean()*0.016
            # loss_dict.update({f'{prefix}/loss_crowd': error})
            # # error = (x_crowd.sum()-crowd_pred).abs() * 0.0005 # 하이퍼파라미터
            # #####################
        else:
            raise NotImplementedError()
        
        ### pred ##########
        # img, x0_partial = self.p_sample(x_noisy, cond, t,
        ...
        # x_start_pred = (x_noisy_pred - extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape) * noise)\
