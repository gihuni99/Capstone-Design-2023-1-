...
@torch.no_grad()
def counting_ver2(image, batch_size, cond):
    image=cv2.resize(image, (512*batch_size, 512))
    image = image[0:504,0:512*batch_size]
    cv2.imwrite("origin2.png", image)
    ret, thresh1 = cv2.threshold(image,127,255, cv2.THRESH_BINARY)
    cv2.imwrite("thresh2.png", thresh1)
    contours, _ = cv2.findContours(thresh1, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    cv2.drawContours(cond, contours, -1, (0,255,0), 3)
    cv2.imwrite("result2.png", cond)
    return len(contours)
...
def ldm_cond_sample(model, batch_size, cv2img):
    ...
    with torch.no_grad():
        ...
        save_image(samples, 'result_zsample.png')
        grid = make_grid(samples)
    
        ndarr = grid.mul(255).clamp_(0, 255).permute(1, 2, 0).to("cpu", torch.uint8).numpy()
        blue,  arr, red = cv2.split(ndarr)
        
        
        # arr = to_arr(samples, batch_size)
        # arr = cv2.bitwise_not(arr)
        
        cv2.imwrite("arr.png", arr)
        ...
        crowd_pred = counting_ver2(arr,batch_size, cv2img)
    return crowd_pred
...
## 2023-06-07T03-02-27_shanghai_amh << 기록
    ...
    config_path = 'logs/2023-06-09T19-18-48_spatial_concat/configs/2023-06-09T19-18-48-project.yaml' # origin: crossattn, 3: concate
    ckpt_path = 'logs/' + '2023-06-09T19-18-48_spatial_concat' + '/checkpoints/last.ckpt' # concate
