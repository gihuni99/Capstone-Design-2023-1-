...
## 2023-06-07T03-02-27_shanghai_amh << 기록
    ...
    ################ 바꿔도 되는 부분 ##################
    timestamp = '2023-06-18T16-44-33'     ###### 타임스탬프
    logname = 'shanghai_A_crossattn/' ##### 트레이닝 이름
    checkpoint = 'last.ckpt' ####### weight 이름
    test_mode = 'A' ######### 테스트 데이터 유형 'A' 또는 'B'
    ####################################################
    
    if test_mode=='A':
        testroot = "/data/Capstone/ShanghaiTech/part_A/test_data/img/"
    else:
        testroot = "/data/Capstone/ShanghaiTech/part_B/test_data/img/"
    logroot = 'logs/'+timestamp+'_' + logname +'/'
    config_path = logroot + 'configs/' +timestamp+'-project.yaml'
    ckpt_path = 'logs/' + timestamp + '_'+ logname + '/checkpoints/' + checkpoint
    ...
    for imgname in list:
        ...
        error_list.append(abs(crowd_pred-(np.sum(gtimg)/205))**2)
        i+=1
        print("gt:", (np.sum(gtimg)/205),"pred:", crowd_pred, "error", abs(crowd_pred-(np.sum(gtimg)/205))**2)
    #print("list(percents):",error_list)
    print("MSE:", math.sqrt(sum(error_list)/len(list)))
