    ...
    with torch.no_grad():
        ...
        if ddim == False:
            for i in range(0, len(intermediates)):
                name = "outputs/x_inter_decoded"+str(i)+".png"
                name2 = "outputs/x_inter"+str(i)+".png"
                save_image((model.decode_first_stage(intermediates[i])+1)/2.,name)
                save_image(((intermediates[i])+1)/2.,name2)
        else:
            for i in range(0, len(intermediates['x_inter'])):
                name = "outputs/x_inter_decoded"+str(i)+".png"
                name2 = "outputs/x_inter"+str(i)+".png"
                save_image((model.decode_first_stage(intermediates['x_inter'][i])+1)/2.,name)
                save_image(((intermediates['x_inter'][i])+1)/2.,name2)
                
        pred_arr = to_arr((samples+1)/2., batch_size)
        # ret, thr = cv2.threshold(pred_arr, 10, 255, cv2.THRESH_TOZERO)
        # cv2.imwrite('outputs/thr.png', thr)
        # print(np.sum(thr))
...
if __name__ == '__main__':
    ...
    gt_name = test_img_name.replace("IMG", "DENSITY").replace("img", "density")
    ...
    inputimg = Image.open(test_img_name)
    ...
    cv2.imwrite('outputs/cv2image.png', cv2image)
    cv2image2 = copy.deepcopy(cv2image)
    ##################################################################3
    text="Ground Truth: " + str(int((np.sum(gtimg)/205)))
    org=(10,500)
    font=cv2.FONT_HERSHEY_SIMPLEX
    size, BaseLine=cv2.getTextSize(text,font,1,2)
    cv2.rectangle(cv2image2,org,(org[0]+size[0],org[1]-size[1]),(0,0,0), -1)
    cv2.putText(cv2image2,text,org,font,1,(255,255,255),2)

    ####################################################################
    cv2.imwrite('outputs/cv2image2.png', cv2image2)
    ...
    print("gt:", (int(np.sum(gtimg)/205)),"pred:", crowd_pred, "Err", abs(crowd_pred-int((np.sum(gtimg)/205))), "SqrErr", abs(crowd_pred-int((np.sum(gtimg)/205)))**2)
    #print("list(percents):",MAE_list)
    print("done.", timestamp, logname, checkpoint)
    print("MAE :", sum(MAE_list)/len(MAE_list))
    print("RMSE:", math.sqrt(sum(RMSE_list)/len(RMSE_list))) # n=1일 때 MAE와 같음
